...
function end_doc($arr = [])
{
    ...
    if (count($js_code_snippets) > 0) {
        ...
        $final_js_output = str_replace(["\n", "\t", "    "], "", $final_js_output);
        if (DEV_MODE == true) {
            file_put_contents("./modules/chunk.js", removeCSSComments($final_js_output));
        }
        ?><script src="<?php echo $pr . "/modules/chunk.js" ?>"></script><?php
    }
    foreach ($arr as $each) {
...
function def_page($object)
{
    ...
    $uri = str_replace("/", "", $object['uri']);
    $style = (isset($object['style'])) ? compress_css_style($object['style']) : "";
    if ($uri === "") {
        $uri = "index";
    }
    ...
    if (DEV_MODE == true) {
        if ($object['css'] !== "") {
            $nf = fopen("./modules/{$file_name}", "w") or die("Error");
            $object['css'] = compress_css_style(str_ireplace(["    ", ": "], ["", ":"], $object['css']));
            fwrite($nf, $object['css']);
            fclose($nf);
        }
    }
    $path = ($object['css'] === "") ? "``" : "`./modules/{$file_name}`";
    script(<<<JS
        yugal.page({render: `{$object['render']}`,uri: `{$object['uri']}`, style: `{$style}`, css: $path,header: `{$object['header']}`,willMount: $willm,didMount: $didm,didUnMount: $didu,willUnMount: $willu});
    JS);
}

$yugal_http_error_page = ["render" => "<h1>ERROR 404</h1>"];
...
{
    return removeCSSComments(str_replace(": ", ":", str_replace(["\n", "\t", "\r\n", "  "], "", $code)));
}
    ...
    ?><!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta http-equiv="X-UA-Compatible" content="IE=edge"><?php
        if ($yugal_styles_to_float != "") {
            if (DEV_MODE == true) {
                ...
                file_put_contents("./modules/chunk.css", removeCSSComments($yugal_styles_to_float));
            }
            echo "<link rel=\"stylesheet\" href=\"$project_root/modules/chunk.css\"/>";
        }
        ?><meta name="viewport" content="width=device-width, initial-scale=1.0"><meta name="theme-color" content="<?php echo $theme_c; ?>" /><link rel="stylesheet" href="<?php echo project_root(); ?>/modules/yugal.css"><?php
           ...
           if (isset($global_yugal_pages[$pathname])) {
               if (isset($global_yugal_pages[$pathname]['header'])) {
                   echo def_comp(makeyugalvalidheader($global_yugal_pages[$pathname]['header']));
               }
           } else {
           ...
           $uri = str_replace("/", "", $present_page['uri']);
           if ($uri === "") {
               $uri = "index";
           }
           $file_name = $uri . ".yugal.css";
           if (isset($present_page['css']) && $present_page['css'] !== "") {
               ?><link rel="stylesheet" type="text/css" href="./modules/<?php echo $file_name; ?>" data-yugal /><?php
           }
           ?></head><body><?php
        ...
        $css_inline = (isset($present_page['style'])) ? compress_css_style($present_page['style']) : "";
        $present_page['render'] = def_comp($present_page['render']);
        script(<<<JS
        yugal.backend = true; 
        JS);
        echo "<div id=\"yugal-root\">{$present_page['render']}</div><style data-yugal-style>{$css_inline}</style><script src=\"{$project_root}/modules/yugal.js\"></script>";
